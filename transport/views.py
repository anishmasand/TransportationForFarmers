...
def registerFarmer(request):
    ...
    farmer = Farmer(name=fullname, email=email, password=password,
 ...
 fullname = request.POST['fullname']
  email = request.POST['email']
   password = request.POST['password']
    ...
    # hashed password
    # hashed_password=make_password(password)
    driver = Driver(name=fullname, email=email, password=password,
                    mobile=mobile, rating=rating, rides_made=0, license_no=license)
>>>>>> > 6100641488d31681c58f3805567b853e76fdbde3
driver.save()
 return render(request, 'transport/loginPageTruck.html')
